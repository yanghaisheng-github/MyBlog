	...
	$.ajax({
		...
		success : function(data) {
			...
			for (var i = 0; i < data.length; i++) {
				var id = data[i].blogCustom.id.toString(8) * data[i].blogCustom.id;
				topBlog += '<li><a href="find/' + id + '.html" title=' + data[i].blogCustom.title + ' target="_blank">' + data[i].blogCustom.title + '</a></li>';
	...
	$.ajax({
		...
		success : function(data) {
			...
			for (var i = 0; i < data.length; i++) {
				var id = data[i].blogCustom.id.toString(8) * data[i].blogCustom.id;
				var time = i * 0.05;
				likeBlog += '<li style="animation-delay:0.' + i + 's" class="animated fadeIn"><i class="ztpic"><a target="_blank" href="find/' + id + '.html" ><img src="' + data[i].blogCustom.images + '"></a></i><b>'
					+ data[i].blogCustom.title
					+ '</b><span>'
					+ data[i].blogCustom.introduction
...
var initBlogByNew = function(page) {
	...
	$.ajax({
		...
		success : function(dataAll) {
			...
			for (var i = 0; i < data.length; i++) {
				arr[i] = data[i].blogCustom.id;
				parm += data[i].blogCustom.id + ",";
				var id = data[i].blogCustom.id.toString(8) * data[i].blogCustom.id;
				var keyword = "";
				if (data[i].blogCustom.keyword != ""
					&& data[i].blogCustom.keyword != null) {
					if (data[i].blogCustom.keyword.search(';') != -1) {
						keyword = data[i].blogCustom.keyword.replace(/;/g,
							"|");
					} else {
						keyword = data[i].blogCustom.keyword;
					}
				}
				newBlog += '<li style="animation-delay:0.' + i + 's" class="animated fadeInDown"><h3 class="blogtitle"><a target="_blank" href="find/' + id + '.html"  >'
					+ data[i].blogCustom.title
					+ '</a></h3><span class="blogpic imgscale"><a href="find/' + id + '.html" title=""><img src="' + data[i].blogCustom.images + '"  /></a></span><p class="blogtext">'
					+ data[i].blogCustom.introduction
					...
					+ Format(data[i].blogCustom.addtime, "yyyy-MM-dd")
					+ '</span><span  class="clicknum">浏览('
					+ data[i].blogCustom.clicknum
		...
		success : function(data) {
			...
			for (var i = 0; i < data.length; i++) {
				...
				clickBlog += '<li style="animation-delay:0.' + i + 's" class="animated fadeIn"><b><a target="_blank" href="find/' + id + '.html">'
					+ data[i].blogCustom.title
					+ '</a></b><p><i><img src="' + data[i].blogCustom.images + '"></i><span>'
					+ data[i].blogCustom.introduction + '</span></p></li>'
			}
			// 初始化数据
